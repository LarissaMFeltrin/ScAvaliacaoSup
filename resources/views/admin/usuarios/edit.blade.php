...
<head>
    ...
    <title>Editar Usuário - SCORDON</title>
    ...
    <section class="py-12">
        <div class="container mx-auto px-6 max-w-4xl">
            ...
            <div class="text-center mb-12">
                ...
                <p class="text-xl text-gray-600 mb-6">Altere os dados do usuário</p>
                ...
                <form method="POST" action="{{ route('admin.usuarios.update', $usuario) }}" class="space-y-6">
                    ...
                    <div class="bg-blue-50 border-l-4 border-blue-400 p-4 mb-6">
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                <h3 class="text-sm font-medium text-blue-800">Editando Usuário</h3>
                                <div class="mt-2 text-sm text-blue-700">
                                    <p>Usuário: <strong>{{ $usuario->aNome }}</strong> ({{ $usuario->nome_role }})</p>
                                    <p>Criado em: {{ $usuario->created_at->format('d/m/Y H:i') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" 
                               name="aEmail" 
                               id="aEmail" 
                               value="{{ old('aEmail', $usuario->aEmail) }}"
                    ...
                    <div>
                        ...
                        <select name="aRole" id="aRole" class="w-full form-select-scordon @error('aRole') border-red-500 @enderror" required>
                            ...
                            <option value="admin" {{ old('aRole', $usuario->aRole) == 'admin' ? 'selected' : '' }}>
                                👑 Administrador - Acesso total
                            </option>
                            <option value="suporte" {{ old('aRole', $usuario->aRole) == 'suporte' ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="nIdEmpresa" id="nIdEmpresa" class="w-full form-select-scordon @error('nIdEmpresa') border-red-500 @enderror">
                            <option value="">Nenhuma (Acesso a todas)</option>
                            @foreach($empresas as $empresa)
                                <option value="{{ $empresa->ID }}" {{ old('nIdEmpresa', $usuario->nIdEmpresa) == $empresa->ID ? 'selected' : '' }}>
                                    {{ $empresa->aNome }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="nStatus" id="nStatus" class="w-full form-select-scordon @error('nStatus') border-red-500 @enderror" required>
                            <option value="0" {{ old('nStatus', $usuario->nStatus) == '0' ? 'selected' : '' }}>
                                ✅ Ativo
                            </option>
                            <option value="99" {{ old('nStatus', $usuario->nStatus) == '99' ? 'selected' : '' }}>
                                ❌ Inativo
                            </option>
                        </select>
                        @error('nStatus')
                    ...
                    <div class="bg-gray-50 p-6 rounded-xl">
                        ...
                        <div class="grid md:grid-cols-2 gap-4 text-sm">
                            <div>
                                <span class="font-semibold text-gray-700">Perfil Atual:</span>
                                <span class="text-gray-600">{{ $usuario->nome_role }}</span>
                            </div>
                            <div>
                                <span class="font-semibold text-gray-700">Status:</span>
                                <span class="text-gray-600">{{ $usuario->isAtivo() ? 'Ativo' : 'Inativo' }}</span>
                            </div>
                            <div>
                                <span class="font-semibold text-gray-700">Criado em:</span>
                                <span class="text-gray-600">{{ $usuario->created_at->format('d/m/Y H:i') }}</span>
                            </div>
                            <div>
                                <span class="font-semibold text-gray-700">Última atualização:</span>
                                <span class="text-gray-600">{{ $usuario->updated_at->format('d/m/Y H:i') }}</span>
                            </div>
                        </div>
                    </div>
